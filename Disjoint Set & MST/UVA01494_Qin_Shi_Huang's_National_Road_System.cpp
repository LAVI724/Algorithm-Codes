...
參考資料:
信宏簡報 week7 需要多看幾次 kruskal
https://morris821028.github.io/2014/05/02/oj/uva/uva-1494/
*/
...
void dfs(int u){
    ...
    for(int i = 0; i < G[u].size(); ++i){
        ...
        // 更新 MST 樹上的點到 v 點的距離
        // 如果 MST 樹上的點到 u 的距離 > u v 之間的距離 ->  MST 樹上的點到 v 的距離 =  MST 樹上的點到 u 的距離 (也就是 MST 經過 v 再走到 u，會這樣更新是因為要找最大距離)
        // 如果 MST 樹上的點到 u 的距離 < u v 之間的距離 ->  MST 樹上的點到 v 的距離 = u v 之間的距離 (也就是 v 走 u 跟 MST 連接)
        for(int j = 0; j < n; ++j){
            if(visited[j]){
